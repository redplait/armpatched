...
{
    dict_t *dnew = (dict_t *)malloc(sizeof *dnew);
...
size_t dnode_t_size = sizeof(struct dnode_t);

static dnode_t *dnode_alloc(void *context)
{
    return (dnode_t *)malloc(dnode_t_size);
}
...
{
    dnode_t *dnew = (dnode_t *)malloc(sizeof *dnew);
    if (dnew) {
	dnew->data = data;
	dnew->parent = NULL;
	dnew->left = NULL;
