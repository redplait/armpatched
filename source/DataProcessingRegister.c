...
static int DisassembleDataProcessingTwoSourceInstr(struct instruction *i, struct ad_insn *out)
{
    ...
    if(strstr(instr_s, "crc")){
        ...
        if(instr_id != AD_INSTR_CMPP){
            prefer_zr = instr_id != AD_INSTR_IRG;
...
static int DisassembleDataProcessingOneSourceInstr(struct instruction *i, struct ad_insn *out)
{
    ...
    if(opcode2 == 0){
        ...
        {
        const char *const *registers = sf == 1 ? AD_RTBL_GEN_64 : AD_RTBL_GEN_32;
...
static int DisassembleLogicalShiftedRegisterInstr(struct instruction *i, struct ad_insn *out)
{
    ...
    const char *const *registers = sf == 0 ? AD_RTBL_GEN_32 : AD_RTBL_GEN_64;
...
static int get_extended_Rm(unsigned option, char *regstr, unsigned Rm,
        unsigned *sz, const char *const **registers){
...
static int DisassembleAddSubtractShiftedOrExtendedInstr(struct instruction *i, struct ad_insn *out, int kind)
{
    ...
    if((instr_id == AD_INSTR_ADDS || instr_id == AD_INSTR_SUBS) && Rd == 0x1f){
        ...
        if(kind == SHIFTED || (kind == EXTENDED && sf == 0)){
            ...
            int _64_bit = get_extended_Rm(option, rm_buf, Rm, &sz, &registers);
