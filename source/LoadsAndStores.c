...
static const struct itab unscaled_instr_tbl[] = {
...
static const struct itab pre_post_unsigned_register_idx_instr_tbl[] = {
    { "strb", AD_INSTR_STRB },
    { "ldrb", AD_INSTR_LDRB },
...
static const struct itab unprivileged_instr_tbl[] = {
...
static int DisassembleLoadAndStoreExclusiveInstr(struct instruction *i,
    ...
    if((size == 0 || size == 1) && (encoding == 2 || encoding == 3 ||
            ...
            registers = AD_RTBL_GEN_64;
       {
        ...
        const char *const *Rs_Rt_Rtbl = AD_RTBL_GEN_32;
        ...
        }
      {
       ...
       static const struct itab tab[] = {
        ...
        const char *const *Rt_Rtbl = AD_RTBL_GEN_32;
        unsigned Rt_Sz = _32_BIT;

        unsigned Rt1 = Rt;
        const char *const *Rt1_Rtbl = AD_RTBL_GEN_32;
        unsigned Rt1_Sz = _32_BIT;

        const char *const *Rt2_Rtbl = AD_RTBL_GEN_32;
...
static int DisassembleLoadAndStoreRegisterInstr(struct instruction *i, struct ad_insn *out, int kind)
{
    ...
    const struct itab *instr_tab = NULL;
    int instr_id = AD_NONE;
    const char *Rt_s;
    const char *Rn_s;
    const char *const *registers = AD_RTBL_GEN_32;
    ...
    instr_s = instr_tab[instr_idx].instr_s;
...
static int DisassembleAtomicMemoryInstr(struct instruction *i,
    ...
    Rs_s = GET_GEN_REG(registers, Rs, PREFER_ZR);
    Rt_s = GET_GEN_REG(registers, Rt, PREFER_ZR);
    Rn_s = GET_GEN_REG(AD_RTBL_GEN_64, Rn, NO_PREFER_ZR);

    use_alias = get_atomic_memory_op(size, V, A, R, o3, opc, Rt, &instr, &alias);
...
static int DisassembleLoadAndStoreRegisterOffsetInstr(struct instruction *i,
    ...
    instr_idx = (size << 3) | (V << 2) |  opc;

    if(OOB(instr_idx, pre_post_unsigned_register_idx_instr_tbl))
        return 1;

    instr = pre_post_unsigned_register_idx_instr_tbl[instr_idx];
