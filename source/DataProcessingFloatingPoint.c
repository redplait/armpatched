...
static int DisassembleCryptographicAESInstr(struct instruction *i, struct ad_insn *out)
{
    ...
    const char *const *registers = AD_RTBL_FP_V_128;
    int sz = _128_BIT;
...
static int DisassembleAdvancedSIMDCopyInstr(struct instruction *i,
    ...
    const char *const *Rd_Rtbl = NULL;
    const char *const *Rn_Rtbl = NULL;
...
static int DisassembleAdvancedSIMDThreeSameInstr(struct instruction *i,
    ...
    Rn = bits(i->opcode, 5, 9);
    Rd = bits(i->opcode, 0, 4);
    ...
    rot = bits(i->opcode, 11, 12);

    if(fp16){
        const char *const *rtbl = AD_RTBL_FP_V_128;
        ...
        if(scalar)
        {
            const char *const *rtbl = AD_RTBL_FP_16;
            unsigned sz = _16_BIT;
...
static int DisassembleAdvancedSIMDTwoRegisterMiscellaneousInstr(struct instruction *i,
    ...
    else if(opcode == 2 || opcode == 6)
   {
        ...
        if(scalar){
            const char *const *Rd_rtbl = rtbls[size];
            const char *const *Rn_rtbl = rtbls[1 + size];
        ...
        if(scalar){
            ...
            const char *const *Rd_rtbl = AD_RTBL_FP_32;
            const char *const *Rn_rtbl = AD_RTBL_FP_64;
...
static int DisassembleAdvancedSIMDThreeDifferentInstr(struct instruction *i,
    ...
    const char *const *Rd_Rtbl = NULL;
    const char *const *Rn_Rm_Rtbl = NULL;
...
static int DisassembleAdvancedSIMDModifiedImmediateInstr(struct instruction *i,
    ...
    if(instr_id == AD_INSTR_FMOV){
        ...
        imm = (uint64)replicate(a, 1, 8) << 56;
        imm |= (uint64)replicate(b, 1, 8) << 48;
        imm |= (uint64)replicate(c, 1, 8) << 40;
        imm |= (uint64)replicate(d, 1, 8) << 32;
        imm |= (replicate(e, 1, 8) << 24) | (replicate(f, 1, 8) << 16) |
    ...
    if(instr_id == AD_INSTR_MOVI && op == 1 && cmode == 14){
        ADD_IMM_OPERAND(out, AD_IMM_ULONG, /*(long *)&*/imm);
...
static int DisassembleAdvancedSIMDShiftByImmediateInstr(struct instruction *i, struct ad_insn *out, int scalar)
{
    unsigned Q = bits(i->opcode, 30, 30);
    unsigned U = bits(i->opcode, 29, 29);
...
static int DisassembleAdvancedSIMDXIndexedElementInstr(struct instruction *i,
    ...
    const char *const *Rd_Rtbl = NULL;
    const char *const *Rn_Rtbl = NULL;
    const char *const *Rm_Rtbl = AD_RTBL_FP_V_128;
...
static int DisassembleConversionBetweenFloatingPointAndIntegerInstr(struct instruction *i,
    ...
    const char *const *Flt_Rtbl = NULL;
    const char *const *Gen_Rtbl = sf == 0 ? AD_RTBL_GEN_32 : AD_RTBL_GEN_64;
...
static int DisassembleFloatingPointDataProcessingOneSourceInstr(struct instruction *i,
    ...
    ADD_FIELD(out, Rd);
    {
    static const struct itab ptype0_tab[] = {
    ...
    static const struct itab ptype1_tab[] = {
        { "fmov", AD_INSTR_FMOV }, { "fabs", AD_INSTR_FABS },
        { "fneg", AD_INSTR_FNEG }, { "fsqrt", AD_INSTR_FSQRT },
        { "fcvt", AD_INSTR_FCVT },
        /* two blanks, idxes [5-6] */
    ...
    static const struct itab ptype3_tab[] = {
        { "fmov", AD_INSTR_FMOV }, { "fabs", AD_INSTR_FABS },
        { "fneg", AD_INSTR_FNEG }, { "fsqrt", AD_INSTR_FSQRT },
        { "fcvt", AD_INSTR_FCVT },
        { "fcvt", AD_INSTR_FCVT },
