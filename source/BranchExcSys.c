...
static const struct itab first[] = {
        { "nop", AD_INSTR_NOP },
...
static const struct itab second[] = {
...
static const struct itab third[] = {
        { "esb", AD_INSTR_ESB },
...
static const struct itab fourth[] = {
...
static int DisassembleSystemRegisterMoveInstr(struct instruction *i,
...

    {
      ...
      if (!sreg_s) {
        free_sreg_s = 1;
        concat(sreg_buf, "S%d_%d_C%d_C%d_%d", 2 + o0, op1, CRn, CRm, op2);
        sreg_s = sreg_buf;
      }
      ...
      if (instr_id == AD_INSTR_MRS) {
        ...
        concat(DECODE_STR(out), "mrs %s, %s", Rt_s, free_sreg_s ? sreg_buf : sreg_s);
      }
      else {
        ADD_REG_OPERAND(out, Rt, _SZ(_64_BIT), PREFER_ZR, _SYSREG(sreg), _RTBL(AD_RTBL_GEN_64));
        ADD_REG_OPERAND(out, Rt, _SZ(_64_BIT), PREFER_ZR, _SYSREG(AD_NONE), _RTBL(AD_RTBL_GEN_64));
